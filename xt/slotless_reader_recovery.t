...
# compiling cbf.h with CBF_READER_SLOTS=1 so the 2nd reader goes slotless.

plan skip_all => 'Linux only' unless $^O eq 'linux';
my $cc = $Config{cc} || 'cc';
system("$cc --version >/dev/null 2>&1") == 0
    or plan skip_all => "no working C compiler ($cc)";
-f 'cbf.h' or plan skip_all => 'run from the distribution root (cbf.h not found)';
...
static struct flags *F; static CbfHandle *L;
int main(void) {
    char eb[256];
    L = cbf_create(NULL, 1000, 0.01, 0600, eb);
    if (!L) { fprintf(stderr, "create: %s\n", eb); return 2; }
    F = mmap(NULL, sizeof *F, PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS, -1, 0);
    memset(F, 0, sizeof *F);
    pid_t a = fork();
    if (a == 0) { cbf_rwlock_rdlock(L);
    ...
    if (b == 0) { cbf_rwlock_rdlock(L);
        ...
        __atomic_sub_fetch(&F->reader_active,1,__ATOMIC_RELEASE); cbf_rwlock_rdunlock(L); _exit(0); }
    ...
    if (c == 0) { cbf_rwlock_wrlock(L);
        ...
        __atomic_store_n(&F->writer_active,0,__ATOMIC_RELEASE); cbf_rwlock_wrunlock(L); _exit(0); }
